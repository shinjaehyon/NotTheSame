...
| Sprint 6 | 1인 1방 제한 + 기본문항 공유 + 수정 기능 + 홈 버튼 | 방 재사용 + 답변 수정 + 네비게이션 개선 |
...
### 완료 기준 (Definition of Done)
- [v] `quiz_rooms` + `questions` 테이블 INSERT 로직
- [v] 6자리 영문/숫자 코드 생성 (중복 방지)
- [v] 커스텀 문항 CRUD UI (Shadcn/UI 컴포넌트 활용)
- [v] 코드 발급 화면 + 클립보드 복사 버튼
...
### 완료 기준 (Definition of Done)
- [v] 코드 입력 → `quiz_rooms` 조회 → 입장 처리
- [v] 문항 단계별 UI (모바일 대응, progress bar 포함)
- [v] 선택지 클릭 → 상태 저장 (useState)
- [v] 제출 시 `responses` INSERT (점수 계산 포함)
- [v] 잘못된 코드 오류 처리
...
### 완료 기준 (Definition of Done)
- [v] 결과 페이지: 점수, 정답 수, 전체 문항 수 표시
- [v] 랭킹 테이블: `responses` 조회 → 점수 내림차순 정렬
- [v] 본인 행 강조 (배경색/볼드 처리)
- [v] Supabase Realtime 구독으로 자동 갱신 (폴링 fallback 포함)
...
### 목표
테스트 후 추가된 요건: 1인 1방 제한, 기본문항 코드 공유, 답변 수정 플로우, "홈으로" 버튼.

### User Stories
- `US-6-1` 호스트로서, 이미 방이 있으면 홈에서 "내 답변 보기" 버튼으로 수정 진입할 수 있어야 한다.
- `US-6-2` 호스트로서, 방을 두 번 생성하려 하면 409 에러와 기존 코드를 받아야 한다.
- `US-6-3` 호스트로서, 수정 모드에서 기존 답변이 폼에 미리 채워져야 한다.
- `US-6-4` 게스트로서, 결과 페이지에서 "홈으로" 버튼으로 메인으로 돌아갈 수 있어야 한다.

### 완료 기준 (Definition of Done)
- [v] DB: `default_answers` 컬럼 + `host_id` UNIQUE 제약
- [v] 기본 10문항 가상 ID 병합, 커스텀만 DB 저장
- [v] 1인 1방 체크 (409 반환)
- [v] 홈 페이지 방 존재 분기
- [v] 수정 플로우 (edit-data 조회 → PUT update)
- [v] "홈으로" 버튼 (완성/수정 페이지, 결과 페이지)
...
| 스프린트 | 상태 | 시작일 | 완료일 |
|---------|------|--------|--------|
| Sprint 1 | ✅ 완료 | 2026-03-13 13:43 | 2026-03-13 14:53 |
| Sprint 2 | ✅ 완료 | 2026-03-13 14:53 | 2026-03-13 15:28 |
| Sprint 3 | ✅ 완료 | 2026-03-13 15:28 | 2026-03-13 15:49 |
| Sprint 4 | ✅ 완료 | 2026-03-13 15:49 | 2026-03-13 16:20 |
| Sprint 5 | ✅ 완료 | 2026-03-13 16:20 | 2026-03-13 16:52 |
| Sprint 6 | ✅ 완료 | 2026-03-13 | 2026-03-13 |
