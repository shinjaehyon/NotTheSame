...
- [x] `npx create-next-app` + TypeScript + Tailwind + Shadcn/UI 설정 완료
- [x] `.env.local`에 Supabase URL/Key 설정
- [x] 4개 테이블 스키마 마이그레이션 적용
- [x] Supabase 연결 확인 (헬스체크 API 라우트)
...
### 완료 기준 (Definition of Done)
- [ ] 닉네임 입력 폼 + 유효성 검사 (빈 값 방지)
- [ ] `crypto.randomUUID()` 또는 DB UUID PK 생성
- [ ] HTTP-Only 쿠키 설정 (`Set-Cookie` 헤더)
- [ ] Next.js Middleware로 미인증 접근 차단 + `/` 리다이렉트
- [ ] `users` 테이블 INSERT 확인
